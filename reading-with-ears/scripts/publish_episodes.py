#!/usr/bin/env python3
"""
Reading with Ears — End-to-End Pipeline
From Inbox to Earbuds in one command.

Finds today's reading-list NotebookLM notebooks, downloads their audio
overviews via the nlm CLI, and uploads each slug to its own Element.fm show (see feeds.json).

Usage:
    python3 publish_episodes.py                  # today
    python3 publish_episodes.py --date 2026-03-19
    python3 publish_episodes.py --download-only  # skip element.fm upload
    python3 publish_episodes.py --upload-only    # skip nlm download (files already exist)
    python3 publish_episodes.py --dry-run        # show what would happen, do nothing
"""
...
def cleanup_old_items(*, audio_dir: Path, cutoff_date_str: str, apply: bool) -> int:
    ...
    allowed_slugs = {str(f.get("slug") or "").strip() for f in all_feeds_with_slug()} or set(
...
def _upload_and_publish_mp3(client: ElementFmClient, mp3_path: Path, title: str, episode_number: int, entry: dict, rich_description: str | None = None) -> bool:
    ...
    entry["elementfm_show_id"] = client.cfg.show_id
    return True
...
def _expected_slugs_for_status(explicit_slugs: list[str] | None) -> list[str]:
    if explicit_slugs:
        return explicit_slugs
    s = enabled_slugs_ordered()
    return s if s else ["news", "think", "professional"]
...
def _is_complete(manifest: dict, slugs: list[str]) -> bool:
    eps = manifest.get("episodes", {})
    return all(eps.get(s, {}).get("published") for s in slugs)


def _print_manifest(manifest: dict, audio_dir: Path, audio_format: str, slugs: list[str]):
...
def show_status(target_date: str, audio_dir: Path, audio_format: str, expected_slugs: list[str] | None = None):
    slugs = _expected_slugs_for_status(expected_slugs)
    # ── Today ──────────────────────────────────────────────────────────────
    ...
    if today_manifest:
        _print_manifest(today_manifest, audio_dir, audio_format, slugs)
        all_published = _is_complete(today_manifest, slugs)
    ...
    for path in manifests:
        ...
        try:
            ...
            continue
        if _is_complete(m, slugs):
            last_complete = m
            break

    if last_complete:
        print(f"  Date: {last_complete.get('date', '?')}")
        _print_manifest(last_complete, audio_dir, audio_format, slugs)
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Reading with Ears — end-to-end pipeline",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 publish_episodes.py                     # run full pipeline for today
  python3 publish_episodes.py --date 2026-03-19   # run for a specific date
  python3 publish_episodes.py --download-only     # nlm download only, skip upload
  python3 publish_episodes.py --upload-only       # element.fm upload only (files must exist)
  python3 publish_episodes.py --dry-run           # preview without doing anything
        """
    )
    ...
    parser.add_argument("--slugs", default=None,
                        help="Comma-separated slugs (default: enabled feeds from config/feeds.json)")
    ...
    if not slugs:
        fail("No slugs to process: enable feeds in feeds.json or pass --slugs.")
        sys.exit(1)
...
""")
    ...
    if args.show_status:
        audio_dir = resolve_audio_dir(cli_audio_dir=args.audio_dir)
        audio_format = resolve_audio_format(cli_audio_format=args.audio_format)
        show_status(target_date, audio_dir, audio_format, expected_slugs=slugs)
        sys.exit(0)
    ...
    if not audio_dir.exists():
        ...
        fail("Create it, or set ~/.config/reading-with-ears/config.json audio_dir, or pass --audio-dir")
        sys.exit(1)
    ...
    if not args.download_only:
        ...
        try:
            ...
            fail(str(e))
            sys.exit(1)
        missing_shows = [s for s in slugs if s not in slug_to_show]
        if missing_shows:
            fail(
                "No elementfm_show_id in feeds.json for slug(s): "
                + ", ".join(missing_shows)
                + " — add or enable the feed in ~/.config/reading-with-ears/feeds.json "
                "(or bundled reading-with-ears/config/feeds.json)."
            )
            sys.exit(1)
        migrate_manifest_episodes_for_per_show_uploads(manifest, slug_to_show, slugs)
        if not args.dry_run:
            try:
    ...
    # ── Step 4: Upload to element.fm (one show per slug from feeds.json) ─
    header("Uploading to element.fm (per-show)")
    uploaded = []
    failed = []

    if not workspace_id or not slug_to_show:
        fail("Internal error: feeds mapping missing for upload step")
        sys.exit(1)

    def _client_for_slug(slug: str) -> ElementFmClient:
    ...
    if not args.dry_run and uploaded:
        print(f"\n  🎧 Episodes published to their Element.fm shows:")
        for slug in uploaded:
            sid = slug_to_show.get(slug)
            if sid:
                print(f"     {slug}: https://app.element.fm/workspaces/{workspace_id}/shows/{sid}")
    print()
