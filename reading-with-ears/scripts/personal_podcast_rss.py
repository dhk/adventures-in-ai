...
Watches the configured audio directory (default: iCloud Personal Podcast folder)
and serves a valid RSS feed over HTTP for any podcast client on your LAN.

Usage:
    python3 personal_podcast_rss.py

Example feed URL: http://<your-mac-ip>:8765/feed.rss

Optional tooling — primary listening for Reading with Ears is via Element.fm / Apple Podcasts;
this server is for local experimentation or apps that can subscribe to a URL on your network.
"""
...
Subscribe in your podcast app (must support adding a URL):
  Paste: {feed_url}

Keep this terminal window open while the server runs.
