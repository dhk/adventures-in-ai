...
class TestPodcastConfig(unittest.TestCase):
    ...
    def test_parse_episode_title_from_filename_known_slug(self):
        ...
        self.assertEqual(title, "News: Signal from Noise - 2026-03-21")
    ...
    def test_parse_episode_title_from_filename_professional(self):
        ...
        self.assertEqual(title, "Professional: The Stack - 2026-03-01")

    def test_parse_episode_title_from_filename_hyphenated_slug(self):
        title = podcast_config.parse_episode_title_from_filename("2026-04-08-vital-signs.mp3")
        self.assertEqual(title, "Vital Signs - 2026-04-08")
    ...
    def test_elementfm_episode_description_fallback(self):
        d = podcast_config.elementfm_episode_description("Thinking: Threads & Deep Dives - 2026-03-24")
        self.assertIn("Reading with Ears", d)
        self.assertIn("Thinking", d)

    def test_elementfm_episode_description_rich(self):
        rich = "How Power Profits From Chaos\n\n• Idea one\n• Idea two\n\nSources: Noahpinion"
        d = podcast_config.elementfm_episode_description("Thinking: Threads & Deep Dives - 2026-03-24", rich)
