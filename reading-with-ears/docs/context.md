...
Built a Claude skill (`reading-with-ears`) that triages starred Gmail, routes emails to NotebookLM notebooks, generates audio, and downloads files. Run interactively via Claude.ai. Required confirming a triage table each time. Good for trust-building; not good for consistency.

**Phase 2 — Python upload script**
Separated the upload step into `publish_episodes.py` — a Python script that finds today's notebooks, waits for audio readiness, downloads, converts M4A → MP3, and publishes to Element.fm. Could run headlessly.

**Phase 3 — Full automation**
Wired Phase 1 and Phase 2 together via `rwe-run.sh`, scheduled via launchd at 6am. Added:
...
Todoist export was later **removed** from scope — to-dos stay in the skill report only; see **`docs/current-design.md`**.
...
- No alerting when the pipeline fails — currently requires checking `~/logs/reading-with-ears/YYYY-MM-DD.log`
...
For the current architecture and principles, see [current-design.md](current-design.md). Historical copy: [archive/prior-key-ideas-2026-04.md](archive/prior-key-ideas-2026-04.md).
