...
**Stale:** Todoist and older automation details superseded — see **`docs/current-design.md`**.
...
**Key files:**
- `~/.claude/skills/reading-with-ears.md` — the skill/prompt that Claude Code reads
- `~/bin/rwe-run.sh` — the shell script invoked by cron
- Logs written to `~/logs/reading-with-ears/YYYY-MM-DD.log`

**Cron schedule (7am daily):**
```
0 7 * * * /Users/davehk/bin/rwe-run.sh
```
