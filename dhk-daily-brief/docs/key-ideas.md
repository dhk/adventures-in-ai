...
launchd also handles TCC (Transparency, Consent, Control) differently — the workaround of caching scripts to `~/.local` and `~/.config` exists because launchd agents can't read `~/Documents` without Full Disk Access.
